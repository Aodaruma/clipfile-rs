...
`TextLayerStringArray` / `TextLayerAttributesArray` がprimary以外を格納する一方、`TextLayerAddAttributesV01` はprimaryを含む同じlittle-endian length-prefixed arrayだった。54/54レイヤーでitem数がtext object数と一致した。parameter 50はmain/additional属性の57/57組に4-byteで存在して値も一致し、同一文書内で重複しなかった。このためtemplate cloneによる追加では、3配列を同時更新し、main/additional属性のparameter 50を同じ新規文書内IDへ置換する。
...
この証拠から、公開write APIは既存Trackをテンプレートに全非identity列をSQLite上で複製する。`_PW_ID`、`MainId`、UUID、layer紐付け、chain、mixer IDだけを独立生成し、mixer完全bodyとsize列の一致を検証する。BINCは曲線値の既知fieldを更新できるが、object metadata、kind別の空曲線構成、open/option flagの一般規則が未確定なため、テンプレート不要の任意Track builderはまだ提供しない。検証用ファイル名・UUID・layer名・作品情報は記録しない。
