...
BINC文字列表を用いて `Type=ImageCelName` の `FCurve` を特定すると、`Frame` (`Single[]`)、`Value` (`Single[]`)、`Tag` (`String[]`) が同数で格納されていた。5件合計10,267キーを境界検証し、`Frame` は有限かつ昇順だった。時刻は60 Hz基準で、表示フレームを `frame × 60 / fps` に変換して直前キーを選ぶと、既存のRust連番出力結果と一致した。

`LayerUuidWithTrack` の16-byte値は `Layer.LayerUuid` のテキスト表現と正規化後に一致し、191トラックすべてを対象レイヤーへ一意に対応できた。公開APIと検証用exporterは、5件すべてでトラック数・キー数が一致した。本文にはタイムライン名、セルタグ、レイヤー名、ファイル別の対応を記録しない。
