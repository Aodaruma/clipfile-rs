...
データありの25,144ブロックをすべて展開したところ、各ブロックは327,680バイト、すなわち `5 × 256 × 256` バイトだった。公開実装の解析と照合すると、先頭の65,536バイトがα、その後が4バイトインターリーブのB/G/R/未使用チャンネルである。ただし、1チャンネルマスクなど別の配置が他のファイルに存在することは公開実装から知られており、本サンプルだけを根拠に5チャンネルへ固定してはならない。
