...
`TimeLine.FirstTrack` と `Track.TrackNextIndex` は5件でそれぞれ60、60、76、46、49行を重複なく全件到達し、0で終端した。`1000` の42行はすべて描画Mipmapと子を持つfolder、`2003` の5行はすべて `LayerType=1584` / `SpecialRenderType=20` のpaper、`4000` の4行はすべて `PlayTime` 曲線・値を持つため、これらを判定helperへ追加した。`4000` の対象layerは4件とも `LayerType=0` のleafで `LayerType=256` のroot直下にあり、2Dカメラ用trackではなかった。`2001` は描画Mipmapを持つleaf 45行だが、42行がraster、3行が別の `LayerType=0` であり、完全な意味はまだ固定しない。
