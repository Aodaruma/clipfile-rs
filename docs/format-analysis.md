...
- 5ファイルの全外部参照列を横断すると、索引に存在しないIDは `Offscreen.BlockData` の14,144件だけだった。すべて40-byteかつ行ごとに一意で、対応する属性は既知の5チャンネル8-bit配置、初期色0、既定値0（14,114件）または1（30件）だった。欠落IDを破損や共通sentinelとは扱わず、外部タイルのないスパース画像として属性の既定値だけで復元する。
...
これにより8-bitの1チャンネル配置は実ファイル検証済みになった。追加の匿名ローカルcorpus 133ファイル・58,487 `Offscreen` 行を集計すると、5チャンネル8-bitが57,496行、1チャンネル8-bitが769行、alphaとgray valueを独立に持つ2チャンネル8-bitが177行、1-bit packed monochromeが45行だった。16-bit/channelと32-bit/channelは0行だった。2チャンネル8-bitはplanar alpha/valueからrow-major `GrayAlpha8` へのdecode/writeを実装した。1-bit packedはmetadata上の存在を確認したが、bit順序と行境界をまだ検証できていないため8-bitと同一扱いにはしない。
...
5サンプルの5,293ブロックコンテナ・163,927ブロックを再集計すると、`BlockStatus` は値0のコンテナ1,322件、値1のコンテナ3,971件で、同一コンテナ内に0と1が混在する例はなかった。追加の匿名生成・アプリ補助コンテナ137件でも混在例はなかった。したがって配列形状はブロック単位だが、観測範囲では外部オブジェクト全体に共通する値として書かれている。意味自体は固定せず、公開APIは個別値に加えて全ブロック一致時だけ値を返す `BlockData::uniform_status` を提供する。

`BlockCheckSum`の生成範囲は、payload object内の4-byte little-endian圧縮長prefixと、それに続くzlib圧縮byte列である。この連結列へ標準Adler-32を適用し、結果を配列へ32-bit big-endianで保存する。匿名ローカル106ファイル・375,717ブロックのうち、非0チェックサム54,649件が全件一致し、不一致は0件だった。圧縮byte列だけへのAdler-32が一致しなかった過去の観測は、この長さprefixを範囲に含めていなかったためである。

全 `BlockCheckSum` を0にした匿名ローカル複製も、検証したCLIP STUDIO PAINTでは正常に開け、別名保存後も0が保持された。このためwriterは後方互換用の明示的な`Zero` modeも保持するが、新規コードでは生成規則を実装した`CspCompatible` modeを推奨する。読み取り時は要素数を検証し、元の`u32`値を損失なく保持する。

## Writer再構築検証

初期`write` featureは、strict validation済みcontainerの埋め込みSQLiteをprivateな書き込み可能DBへ複製し、既知top-level layoutを新規出力へ再構築する。外部本体の長さが変わる場合は、後続chunk位置、`CHNKHead`のSQLite絶対位置、SQLiteの全`ExternalChunk.Offset`を再計算する。未知SQLite列と変更しない外部本体は保持する。

ローカルコーパス5件（約37～351 MB）を変更せず再構築し、全件で入力と出力がbyte-for-byte同一になった。さらに匿名の最小アニメーション文書についてSQLiteのlayer名だけを変更し、Rust側の再オープン・SQLite `quick_check`・external index検証に加え、CLIP STUDIO PAINTでキャンバス、タイムライン、変更後のlayer表示を確認した。固有のファイル名、layer名、ユーザーパスは記録せず、生成物と検証scriptは`tester/`だけに保持する。

既存BlockDataについては、1ブロックの `channels × width × height` と展開byte数を照合し、zlib再圧縮して本体を再構築する限定APIを追加した。対象外ブロックの圧縮payload、parameter、status、checksumを保持し、再構築後のBlockDataを再解析する。合成テストは既存payload、空ブロック、連続編集、失敗時の保留結果維持を含む。匿名の最小ラスター文書を公開Rust APIだけで変更し、Rust側の完全検証とCLIP STUDIO PAINTでの警告なし再読込を確認した。ファイル名、layer名、ユーザーパス、外部IDは記録していない。

現在のwriterは任意の新規objectを作る一般的なencoderではない。未知top-level chunkを持つ入力はstrict validationで拒否し、完全なopaque external body差し替え時は関連する意味上の整合性を呼び出し側が管理する。BlockData APIは既存parameterに合うnative byte列の置換に限定するが、画像全体の変更tileは一括再構築し、対応する全`Offscreen.Attribute.BlockSize`をtransaction内で同期する。
...
匿名の最小ベクターレイヤーでは `LayerType=0` と `VectorObjectList` 1行を確認した。`VectorData` は40-byteの外部IDで、`ExternalChunk` から268-byteの `CHNKExta` 本体へ解決できた。追加差分から、対応形式のbodyは92-byte stroke headerと88-byte point recordの連結として末尾まで走査でき、point座標とstroke/point bounding boxの位置を検証した。その他のbrush、pressure、opacity、flagを含むfieldの意味は固定しない。

既存stroke recordの完全clone、座標とbounding boxだけを平行移動したclone、全strokeを削除した空bodyは、匿名ローカル出力をアプリで開き直せた。header末尾の不透明4-byte値をclone元と同じまま重複させても同じ検証条件では受理された。ただし同レイヤーの通常描画Mipmap、thumbnail、previewはvector bodyと別のcacheであり、stroke編集だけでは再生成されない。公開write APIは対応layoutの平行移動・template clone・削除に限定し、未知fieldをbyte-for-byte保持する。別layoutやbrushのゼロ生成は拒否する。

公開参考コーパスの定規サンプルは18レイヤーを持ち、8件が `RulerVectorIndex` から同じlayer・canvasの `VectorObjectList` を参照し、10件が `SpecialRulerManager` を参照した。managerの9個の `First*` と各定規表の `NextIndex` をたどると、parallel 1、curve parallel 1、multiple curve 1、radial line 1、radial curve 1、concentric circle 1、guide 7、perspective 1、symmetry 2の計16定規へ欠落・循環なく到達した。

curve系2件の `PointData` は24-byteのbig-endian headerに続く5点または6点で、各点はdouble精度XYと32-bit thicknessだった。perspective定規は1件の `RulerVanishPoint` を持ち、guide BLOB長は `GuideNumber × GuideDataSize` と一致した。公開APIはmanager・定規・消失点の所有layer/canvas、chain全到達、有限な座標、件数・BLOB上限を検証する。未解明のcurve header 4語とguide BLOB、ベクター定規の外部本体は元値を保持する。

匿名の単一テキストレイヤーでは `LayerType=0`、`TextLayerType=0` を確認した。本文は `TextLayerString` のUTF-8 BLOB、対応する書式情報は1,029-byteの `TextLayerAttributes` BLOB、`TextLayerAttributesVersion=1` だった。`TextLayerStringArray` と `TextLayerAttributesArray` はこの例ではNULLである。公開APIは本文のUTF-8、追加配列のlittle-endian長と境界、本文・属性件数の一致を検証し、未解明の属性本体は元バイト列として返す。検証に用いた本文自体は記録しない。

匿名・公開参考コーパスのうちtext schemaを持つ16ファイルを集計すると、54レイヤー・57objectがあり、属性versionは全件1、属性parameterは `u32 LE ID + u32 LE payload長 + payload` の繰り返しとして全件末尾まで一致した。parameter順序は2種類あり、48個の基本集合と、そこへ12個を加えた集合だった。この違いとrun・font・geometry payloadの可変長から、空の属性を固定値だけで生成する一般builderはまだ安全と判断できない。

`TextLayerStringArray` / `TextLayerAttributesArray` がprimary以外を格納する一方、`TextLayerAddAttributesV01` はprimaryを含む同じlittle-endian length-prefixed arrayだった。54/54レイヤーでitem数がtext object数と一致した。parameter 50はmain/additional属性の57/57組に4-byteで存在して値も一致し、同一文書内で重複しなかった。このためtemplate cloneによる追加では、3配列を同時更新し、main/additional属性のparameter 50を同じ新規文書内IDへ置換する。削除でも同じ3配列を同期し、primary削除時は次のobjectをmain列へ昇格できる。空text layerの表現は一般化せず、最後の1 objectは保持する。

parameter 42は16-byte、64は32-byteで57/57objectに存在する。ただし64を8個のlittle-endian整数、42を4個の整数とみなした単純な100倍min/max関係は0/57で一致せず、座標系と変換順序を確定できなかった。clone APIはgeometryもそのまま複製し、自動移動を行わない。

公開参考コーパスの補正レイヤー32件は、全件 `LayerType=4098`、`FilterLayerInfo` はbig-endianのkindとsection長で始まった。kind 1～9がすべて含まれ、内訳は明るさ・コントラスト4、レベル補正5、トーンカーブ6、色相・彩度・明度3、カラーバランス4、階調反転1、ポスタリゼーション3、二値化3、グラデーションマップ3だった。固定長parameter、32 channel recordのレベル・トーンカーブ、可変stop・倍精度curve pointを持つグラデーションマップをRust APIで全件末尾まで復号した。公開APIはpayloadサイズとchannel・stop・point数を制限し、raw固定小数点wordと元payloadを保持する。未知kindも共通section境界を検証後、payloadを不透明なまま返す。

## 通常アニメーション

既存コーパス5件はいずれも `TimeLine` 1行を持ち、fpsは24、bank IDは2だった。`Track` は合計291行で、セル選択に対応する `TrackKind=2000` は191行だった。各 `TrackActionMixer` は40-byteの外部IDからlittle-endian長付きzlibへ解決でき、展開後は `cmt 0100binc` または `cmt 0110binc` で始まるBINCデータだった。

BINC文字列表を用いて `FCurve` を列挙すると、`Frame` / `Value` に加えて `Interp`、`LeftSlope`、`RightSlope`、任意の `Tag` と `ReviseConstant` が同じキー数で格納されていた。`TrackKind=2000` には合計256本の `ImageCelName` 曲線（12,324キー）があり、1トラックに複数曲線を持つ例もある。従来互換のセル選択ビューは各トラックの先頭曲線、191トラック合計10,267キーを使う。`Frame` は有限かつ昇順で、時刻は60 Hz基準だった。表示フレームを `frame × 60 / fps` に変換して直前キーを選ぶと、既存のRust連番出力結果と一致した。
...
匿名のanimation・keyframe・2Dカメラ差分8件で、Track表は `_PW_ID`, `MainId`, `BankId`, `ItemId`, `TrackNextIndex`, primary/secondary mixer size・ID, `TrackValueMap`, open flags, `TrackUuid`, `LayerUuidWithTrack`, `TrackKind`, `TrackOptionFlag` の16列だった。`_PW_ID` は `INTEGER PRIMARY KEY AUTOINCREMENT`、`idxTrack` は非uniqueな`MainId`索引である。新しいcamera trackでは`MainId`が既存最大値の次、`BankId`が対象`TimeLine.BankId`、既存末尾の`TrackNextIndex`が新ID、clone側のnextが0になった。

`TrackUuid`と`LayerUuidWithTrack`は16-byte BLOBで、後者は対象`Layer.LayerUuid`のtext UUIDを正規化した値と一致した。primary/secondaryの各IDは40-byteで、外部bodyはlittle-endian圧縮長付きzlib、size列はそれぞれ展開後 `0100binc` / `0110binc` 長と一致する。open flagとoption flagにはUI状態・kind依存の差があり、意味を一般生成規則として固定できない。

この証拠から、公開write APIは既存Trackをテンプレートに全非identity列をSQLite上で複製する。`_PW_ID`、`MainId`、UUID、layer紐付け、chain、mixer IDだけを独立生成し、mixer完全bodyとsize列の一致を検証する。kind `2000`では完全なtemplate clone後に、唯一の`ImageCelName` curveを呼出側の非空key列へ正規化できる。既存FCurveは、現在存在する全per-key配列を同期してkeyを追加・削除し、対応secondary recordも同じindexで更新できる。空curveは一般化しない。Track row削除はtimeline chainを修復するが、opaque mixer外部本体は別参照を否定できないため回収しない。

BINCのobject metadata、kind別の空曲線構成、open/option flagの一般規則は未確定である。このためtemplate不要の任意Track builderは提供しない。検証用ファイル名・UUID・layer名・作品情報は記録しない。
...
- ベクターbrush/stroke headerの未知field、テキストstyle/geometry属性、3D、定規の未命名header・guide BLOB、補正以外の特殊レイヤーの完全な意味
