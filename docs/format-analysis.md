...
匿名の短いストローク2件と部分マスクでも候補不一致を再確認した。さらに使用したCRCカタログの全32-bit variant、ブロックヘッダーを含む範囲、CityHash、FarmHash、MetroHash、SpookyHash、MurmurHash2/3、FNV、Boost hash combine、xxHash、暗号学的ハッシュの32-bit部分も比較したが一致しなかった。新規保存した最小ファイルでは、データ有無にかかわらず `BlockStatus` が1になる例を観測したため、この値をデータ有無フラグとして扱うこともできない。

一方、匿名の検証用文書について全 `BlockCheckSum` を0にしたローカル複製は、CLIP STUDIO PAINTで正常に開け、表示内容も維持された。その文書を別名保存しても0値はそのまま保持された。このアプリ版では0が受理されるため、アルゴリズム未解明は書き込みの絶対的な阻害条件ではない。ただしバージョン間互換性は未確認である。writerは変更しないブロックとチェックサムをそのまま保存し、既存BlockDataの1ブロックを再圧縮する場合だけ `BlockChecksumMode::Zero` の明示を要求する。読み取り時は要素数を検証し、値自体は不透明な `u32` として保持する。
...
現在のwriterは一般的なsemantic block encoderではない。未知top-level chunkを持つ入力はstrict validationで拒否し、完全なopaque external body差し替え時も、関連するサイズ列や意味上の整合性を自動生成しない。BlockData APIも既存parameterに合うnative byte列の1ブロック置換に限定する。
