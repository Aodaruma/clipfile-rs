...
一方、匿名の検証用文書について全 `BlockCheckSum` を0にしたローカル複製は、CLIP STUDIO PAINTで正常に開け、表示内容も維持された。その文書を別名保存しても0値はそのまま保持された。このアプリ版では0が受理されるため、アルゴリズム未解明は書き込みの絶対的な阻害条件ではない。ただしバージョン間互換性は未確認である。将来のwriterは、変更しないブロックの値を保存し、新規・再圧縮ブロックでは0を明示する方式をopt-inにして複数バージョンで検証する。読み取り時は要素数を検証し、値自体は不透明な `u32` として保持する。
