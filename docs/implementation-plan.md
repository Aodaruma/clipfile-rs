...
- 未知列を保持し、ID・UUID・tree link・cache参照を再生成するplain raster layerのtemplate clone
- 文字幅を維持したtext本文置換、main/additional属性を同期するtemplate-based object追加、primary昇格を含むobject削除
- 検証済みvector layoutの平行移動、template stroke追加、stroke削除、型付き参照によるopaque body置換
- 既存animation値・curve key・cel tag更新、key追加削除、独立ID・mixerを持つtemplate-based Track/image-cel Track clone、chainを修復するTrack削除
...
- 任意の画像object、vector brush/stroke、text style、animation Track/curve metadataをtemplateなしでゼロから生成するencoder
- group・mask付きraster layer clone、派生mipmap/thumbnail/canvas previewのsemantic再生成
- text geometry移動、異なる文字幅へのrun再構築、空text layerの表現
- vector render cache再生成、animation空curve、不要と証明できるmixer orphanの回収
- Track kindとlayer種別の互換性自動検証
...
3. `0.5.x`: 既存構造を対象とした画像全体、vector、text、animationのsemantic writeとtemplate clone
4. `1.0.0`候補: 画像・vector・animationの新規生成範囲を詰め、対応・非対応範囲、公開API互換方針、複数アプリ版での書き込み互換性を固定する
5. 低優先の継続対象: time-lapse、3Dの読み書きと、現在rawで保持する属性の型付け
