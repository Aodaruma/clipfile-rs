...
ローカル実ファイル5件で変更なしの完全一致を確認した。匿名のSQLite layer名だけを変えた出力はCLIP STUDIO PAINTで開き直し、キャンバス、タイムライン、変更後のlayer表示を確認した。検証ファイル、script、出力は`tester/`だけに置き、追跡していない。

残る作業は次のとおり。

- ブロックチェックサムの生成または明示的な0互換modeを複数アプリ版で検証
- raster、vector、text、animation、time-lapse、3Dのsemantic external-body encoder
- 未知top-level chunkを含むcontainerの損失なしラウンドトリップ
- 元ファイルを置換する場合の、同一filesystem上のatomic commit API
- CLIP STUDIO PAINT複数バージョンでの開き直し試験

匿名の最小文書では全`BlockCheckSum`を0にしてもアプリで開け、別名保存後も0が保持された。ただし初期writerは新規blockや0チェックサムを合成せず、変更しない値をそのまま保存する。これにより未検証の互換規則を正しいものとして公開APIへ固定しない。
