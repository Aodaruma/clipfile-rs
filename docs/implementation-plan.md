...
### 8. `write` — 初期低レベルAPI実装済み（`write` feature）
...
- 既存BlockData内の1ブロックを、検証済みnative byte列からzlib再圧縮
- `BlockChecksumMode::Zero` を明示した場合だけ再圧縮ブロックのchecksumを0として生成
...
- ブロックチェックサムの生成規則を解明するか、明示的な0互換modeを複数アプリ版で検証
- 画像全体、vector、text、animation、time-lapse、3Dのsemantic external-body encoder
- 未知top-level chunkを含むcontainerの損失なしラウンドトリップ
- 元ファイルを置換する場合の、同一filesystem上のatomic commit API
- CLIP STUDIO PAINT複数バージョンでの開き直し試験

匿名の最小文書では全`BlockCheckSum`を0にしてもアプリで開け、別名保存後も0が保持された。writerはこの観測を既定動作にはせず、`BlockChecksumMode::Zero` を明示した既存BlockDataの1ブロック再圧縮にだけ利用する。変更しないブロックは元の値を保存し、複数アプリ版で未検証の互換規則を一般的な正解として固定しない。
...
4. `0.4.x`: アニメーション・タイムラプスの読み取り（primary/secondary FCurve、2Dカメラ、タイムラプスBLOBの安全な到達は実装済み）
